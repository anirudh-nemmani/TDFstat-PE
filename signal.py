...
class Signal:
    ...
    def __init__(self,params ,ini_path: str):
    ...
    def amplitude_modulation(self):
        ...
        a1  = (1/16)*np.sin(2*)*(3 - np.cos(2*self.lambda))*(3- np.cos(2*self.dec))*np.cos(2*(modulation))
        ...
        b4 = 0.25*np.sin(2*self.gamma)*np.sin(2*self.lambda)*np.cos(self.dec)*np.sin(modulation)
